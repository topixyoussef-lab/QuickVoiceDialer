...
class SettingsRepository @Inject constructor(
    ...
    private object Keys {
        ...
        val DEFAULT_SERVER_URL = ""  // User must configure in Settings → Wi-Fi calls
        val OLD_DEFAULT_SERVER_URL = "wss://yukon-article-filled-reid.trycloudflare.com/signaling"
        val OLD_LOCAL_DEFAULT_URL = "ws://192.168.1.6:8080/signaling"
        ...
        val VOIP_SERVER_URL = stringPreferencesKey("voip_server_url")
    ...
    val voipServerUrl: Flow<String> = dataStore.data.map {
        val saved = it[Keys.VOIP_SERVER_URL]
        when {
            saved.isNullOrBlank() || saved == Keys.OLD_DEFAULT_SERVER_URL || saved == Keys.OLD_LOCAL_DEFAULT_URL -> Keys.DEFAULT_SERVER_URL
            else -> saved
        }
    }

    suspend fun setVoipServerUrl(url: String) {
        dataStore.edit { it[Keys.VOIP_SERVER_URL] = url.trim().removeSuffix("/") }
    }
