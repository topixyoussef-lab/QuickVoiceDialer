 ...
 * AudioDeviceModule. The WebSocket signaling runs against a self-hosted server and the
 ...
 * live call, and through the signaling server (voice message) when the peer is offline.
 */
@Singleton
class VoipCallManager @Inject constructor(
    ...
    /** Reads the STUN + TURN configuration from Settings and rebuilds the ICE server list. */
    private suspend fun resolveIceServers() {
        val turnUrl = settings.voipTurnUrl.first().trim()
        if (turnUrl.isBlank()) {
            resolvedIceServers = DEFAULT_STUN
            return
        }
        val username = settings.voipTurnUsername.first().trim()
        val password = settings.voipTurnPassword.first()
        val turn = PeerConnection.IceServer(
            turnUrl,
            username.ifEmpty { null },
            password.ifEmpty { null },
        )
        resolvedIceServers = DEFAULT_STUN + listOf(turn)
    }
